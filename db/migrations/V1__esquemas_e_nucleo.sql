-- V1: esquemas, privilégios padrão e entidades compartilhadas (nucleo).
-- Migrações contêm só estrutura e dados de referência; dados de pesquisa
-- entram por db/carga-inicial.
...
COMMENT ON SCHEMA auditoria IS 'Trilha de auditoria de escrita. Só o invips_admin altera.';
COMMENT ON SCHEMA staging IS 'Área temporária da carga inicial; tabelas só com text, recriadas a cada carga.';

-- Leitura: esquemas de dados
GRANT USAGE ON SCHEMA nucleo, infraestrutura, noticias TO invips_leitor;
-- Curador: DML nos esquemas de dados, leitura da auditoria, staging livre
GRANT USAGE ON SCHEMA nucleo, infraestrutura, noticias, auditoria TO invips_curador;
GRANT USAGE, CREATE ON SCHEMA staging TO invips_curador;

ALTER DEFAULT PRIVILEGES IN SCHEMA nucleo, infraestrutura, noticias
  GRANT SELECT ON TABLES TO invips_leitor;
ALTER DEFAULT PRIVILEGES IN SCHEMA nucleo, infraestrutura, noticias
  GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO invips_curador;
ALTER DEFAULT PRIVILEGES IN SCHEMA nucleo, infraestrutura, noticias
  GRANT USAGE, SELECT ON SEQUENCES TO invips_curador;
ALTER DEFAULT PRIVILEGES IN SCHEMA auditoria
  GRANT SELECT ON TABLES TO invips_curador;
